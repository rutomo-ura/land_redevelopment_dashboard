...
Use `Vacant Land Redevelopment Explorer Dashboard` as the canonical URA Maps entry. It embeds the GitHub Pages application and should be shared with the intended audience. The current GitHub Pages bundle includes owner names and EPP workflow attributes, and GitHub Pages itself is reachable without application authentication. Do not share the ArcGIS item with `Everyone` as a substitute for data protection; move the web app and data behind authenticated hosting before adding confidential or protected operational fields.
...
1. The GitHub Pages dashboard loads for the intended staff-review audience.
...
The web app already attempts sources in this order: hosted feature layer, URA ArcGIS GeoJSON data URL, URA ArcGIS GeoJSON portal item, then the staff-review GitHub Pages GeoJSON bundle.
