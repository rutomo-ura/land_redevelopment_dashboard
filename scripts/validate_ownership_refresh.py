...
def load_ownership_summary_counts(key: str) -> dict[str, int]:
...
def main() -> None:
    ...
    dashboard_summary_counts = load_ownership_summary_counts("groups")
    reference_summary_counts = load_ownership_summary_counts("referenceGroups")
    ...
    reference_summary_mismatches = []
    dashboard_summary_mismatches = []
    for group in OWNERSHIP_GROUPS:
        summary_value = reference_summary_counts.get(group, 0)
        reference_value = reference_counts.get(group, 0)
        if summary_value != reference_value:
            reference_summary_mismatches.append(
                {
        ...
        dashboard_value = dashboard_summary_counts.get(group, 0)
        parcel_value = public_counts.get(group, 0)
        if dashboard_value != parcel_value:
            dashboard_summary_mismatches.append(
                {
                    "ownership_group": group,
                    "dashboard_summary": dashboard_value,
                    "dashboard_parcels": parcel_value,
                    "difference": dashboard_value - parcel_value,
                }
            )
    ...
    write_csv(
        QA_DIR / "dashboard_ownership_summary_mismatches.csv",
        dashboard_summary_mismatches,
        ["ownership_group", "dashboard_summary", "dashboard_parcels", "difference"],
    )
    write_csv(
    ...
    passed = (
        not reference_summary_mismatches
        and not dashboard_summary_mismatches
    ...
    lines = [
        ...
        f"| Ownership reference-summary mismatches | {len(reference_summary_mismatches):,} |",
        f"| Dashboard ownership-summary mismatches | {len(dashboard_summary_mismatches):,} |",
